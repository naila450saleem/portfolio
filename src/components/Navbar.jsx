...
export default function Navbar() {
  ...
  className="text-4xl font-extrabold cursor-pointer tracking-wider leading-none bg-gradient-to-r from-green-600 to-black bg-clip-text text-transparent drop-shadow-lg"
...
>
  Portfolio
        ...
        <ul className="hidden lg:flex items-center gap-x-10 font-semibold text-green-600 absolute left-1/2 transform -translate-x-1/2">
          {navItems.map((item) => (
            <motion.li key={item.id} className="group" whileHover={{ scale: 1.1 }}>
              <button
              ...
              <motion.span
                className="w-0 transition-all duration-300 group-hover:w-full h-[2px] bg-green-600 flex"
                layout
              ></motion.span>
            </motion.li>
          ))}
